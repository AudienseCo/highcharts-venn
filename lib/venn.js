...
seriesTypes.venn = Highcharts.extendClass(Highcharts.Series, {
  ...
  translate: function(){
    ...
    this.sortedAmounts=[parseInt(followA), parseInt(followB), parseInt(followC)];
    this.sortedAmountsInter= new Array(5);
    ...
    var rA = this.rA = scale(this.sortedAmountsInter[0]/(this.sortedAmountsInter[0]/10));
    var rB = this.rB = scale(this.sortedAmountsInter[1]/(this.sortedAmountsInter[0]/10));
    var rC = this.rC = scale(this.sortedAmountsInter[2]/(this.sortedAmountsInter[0]/10));
    var ab = this.ab = scale(this.sortedAmountsInter[3]/(this.sortedAmountsInter[0]/10));
    var ac = this.ac = scale(this.sortedAmountsInter[4]/(this.sortedAmountsInter[0]/10));
    var cb = this.cb = scale(this.sortedAmountsInter[5]/(this.sortedAmountsInter[0]/10));
    ...
    this.lineInterAC();
    this.lineInterAB();
    this.points[0].options.shapeType = 'circle';
    this.points[0].options.shapeArgs = {
          x: this.cA.x,
          y: this.cA.y,
          r: rA,
          fill: this.points[0].color,
          opacity: 0.8
        };
    this.points[1].options.shapeType = 'circle'
    this.points[1].options.shapeArgs = {
          x: this.cB.x,
          y: this.cB.y,
          r: rB,
          fill: this.points[1].color,
          opacity: 0.8
        };
    this.points[2].options.shapeType = 'circle';
    this.points[2].options.shapeArgs = {
          x: this.cC.x,
          y: this.cC.y,
          r: rC,
          fill: this.points[2].color,
          opacity: 0.8
...
sortCircle:function(array, followAB,followAC, followBC){
	...
	var arraySorted= new Array(5);
	
	arraySorted[0]=array[0];
	arraySorted[1]=array[1];
	arraySorted[2]=array[2];
	
	arraySorted[3]=followAB;
	arraySorted[4]=followAC;
	arraySorted[5]=followBC;
	return  arraySorted;
...
// C  CIRCLE
  ...
  calculateC: function(){
    ...
    if(rC!=null){
      this.rAC = rA+rC-ac*2;
      this.rCB = rB+rC-cb*2;
      var intsec = this.intersection(cA.x, cA.y, this.rAC, cB.x, cB.y, this.rCB);
      console.log("intsec"+intsec);
		...
		if(intsec[3]==cA.y && intsec[1]==cA.x && ac!=this.rC && this.cb==0){
		
			
			intsec[1]=this.cA.x-rA-rC+2*ac;
			 this.ACInterceptionBehaviour=1;
		} 
       
	 //right interception CB
      
      if(intsec[3]==cA.y && intsec[1]==cA.x && ac!=this.rC &&  this.ac==0){
		  
		  
		  intsec[1]=this.cB.x+this.rB+rC-2*cb;
		  this.BCInterceptionBehaviour=1;
	  }
      
       //in the middle of B and A
      if(intsec[3]==cA.y && intsec[1]==cA.x && ac!=this.rC ){
          
          intsec[3]=cA.y;
   
          intsec[1]=cA.x+this.rA+this.cb-this.ac;
  ...
  drawPoints: function(){
    ...
    each(this.data, function (point) {
      console.log('point', point.visible);
      var graphic = point.options.graphic,
        shapeArgs = point.options.shapeArgs,
        shapeType=point.options.shapeType;
      if(graphic){
		...
		if(shapeType=='path'){
			point.options.graphic = renderer.path(shapeArgs).add(series.group);
		}else{
        point.options.graphic = renderer.circle(shapeArgs).add(series.group);
